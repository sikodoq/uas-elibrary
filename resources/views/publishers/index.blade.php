        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">{{ __('List Data Publishers') }}</h3>
                </div>
                <div class="box-header mt-2 ml-4">
                    <a href="{{ route('publishers.create') }}" class="btn btn-primary" role="button" title="Add Data"><i
                            class="nav-icon far fa-plus-square"></i> Add Publisher</a>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-danger btn-sm" aria-label="Delete" type="submit"
                                            onclick="deleteAlert('{{ $publisher->id }}', 'Delete Publisher {{ $publisher->name }}')"><i
                                                class="fas fa-trash">
                                            </i> Delete</button>
                                        <form action="{{ route('publishers.destroy', $publisher->id) }}" method="POST"
                                            class="d-inline" id="Delete{{ $publisher->id }}">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    </td>
